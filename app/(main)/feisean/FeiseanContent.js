...
import { STATE_NAMES, MONTH_NAMES, groupByMonth, groupByState } from './data';

// Phase 2: cards now render a real og:image when one was found at fetch
// time (see ./og-image.js). When no image is available — site missing
// og tags, slow fetch, error — `hero_image_url` is null and the card
// falls back to the celtic-ring CSS gradient placeholder. The card
// anatomy is identical either way so the grid doesn't shift.
function FestivalCard({ festival, isDynamic }) {
  ...
  const websiteLabel = festival.website
    ...
    : null;
  const hero = festival.hero_image_url;
  ...
  return (
    ...
    >
      ...
      <div className="relative aspect-[3/2] bg-gradient-to-br from-tarheel/40 via-cobalt/30 to-gc-dark overflow-hidden">
        {hero ? (
          // Use a plain <img> (not next/image) because we want it loadable
          // from any third-party origin without an `images.remotePatterns`
          // entry per site — 35+ ASGF hosts and growing. Cover-crop so the
          // image always fills the card regardless of source aspect ratio.
          <img
            src={hero}
            alt={festival.name}
            loading="lazy"
        ...
        {isDynamic && (
          <div className="absolute top-2 right-2 px-2 py-1 rounded-md bg-black/60 backdrop-blur-sm text-[10px] tracking-[0.15em] uppercase text-white/90 font-display z-10">
            via sruth.
          </div>
        )}
      </div>
  ...
  return (
    <section className="mb-12">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-5">
        {festivals.map((f) => (
          <FestivalCard key={f.id} festival={f} isDynamic={f._source === 'sruth'} />
        ))}
      </div>
    </section>
  );
...
export default function FeiseanContent({ festivals }) {
