...
export const REACTION_KINDS = ['slainte', 'gaol', 'gaire', 'iongnadh', 'taing'];
...
export async function attachReshares(rows) {
  ...
  const { data, error } = await supabaseAdmin
    .from('gc_posts')
    .select('id, body, media, created_at, status, deleted_at, author:gc_profiles!gc_posts_author_id_fkey(id, handle, display_name, avatar_url)')
    .in('id', originIds);
  if (error) throw error;
  ...
  for (const o of data || []) {
    ...
    byId.set(o.id, gone
      ...
      : { id: o.id, body: o.body, media: o.media || null, created_at: o.created_at, author: publicProfile(o.author) });
  }
